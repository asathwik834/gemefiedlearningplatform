...
import { ArrowLeft, CheckCircle, Percent, Timer, XCircle } from "lucide-react";
import { useTranslation } from "../../hooks/useTranslation";
const QUESTIONS = [
  { id: 1, q: "Find x: 2:3 = x:12", options: ["6", "7", "8", "9"], correct: 0 },
  { id: 2, q: "Simplify the ratio 18:24", options: ["3:4", "4:3", "6:8", "9:12"], correct: 0 },
  { id: 3, q: "If a recipe needs 2 cups sugar for 8 cookies, how many cups for 20 cookies?", options: ["3", "4", "5", "6"], correct: 2 },
  { id: 4, q: "A map scale is 1:100,000. 3 cm on map equals how many km?", options: ["1 km", "3 km", "30 km", "300 km"], correct: 2 },
  { id: 5, q: "What percent is 15 out of 60?", options: ["15%", "20%", "25%", "30%"], correct: 2 },
  { id: 6, q: "Find x: 5/x = 10/20", options: ["2", "3", "4", "5"], correct: 2 }
];
const ProportionLab7 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("proportion-lab7", scoreVal, { title: "Proportion Lab7", subject: "Mathematics" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" /> {t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><Percent className="w-10 h-10 text-teal-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700 mb-1">{t("totalPoints")}: {score}</p>
