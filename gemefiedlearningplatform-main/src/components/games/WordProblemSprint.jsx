...
import { ArrowLeft, BookOpenCheck, CheckCircle, Timer, XCircle } from "lucide-react";
import { useTranslation } from "../../hooks/useTranslation";
const QUESTIONS = [
  { id: 1, q: "Ravi has 36 marbles. He shares them equally among 4 friends. How many does each get?", options: ["6", "7", "8", "9"], correct: 2 },
  { id: 2, q: "A book has 120 pages. Riya reads 15 pages per day. How many days to finish?", options: ["6", "7", "8", "9"], correct: 2 },
  { id: 3, q: "A shop sells pencils at \u20B95 each. How many pencils can you buy with \u20B945?", options: ["7", "8", "9", "10"], correct: 2 },
  { id: 4, q: "A tank holds 60 liters. It is filled at 4 liters/min. How long to fill?", options: ["10 min", "12 min", "14 min", "15 min"], correct: 3 },
  { id: 5, q: "One pizza is cut into 8 slices. You eat 3 slices. What fraction did you eat?", options: ["3/5", "3/8", "5/8", "8/3"], correct: 1 }
];
const WordProblemSprint = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("word-problem-sprint", scoreVal, { title: "Word Problem Sprint", subject: "Science" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" /> {t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><BookOpenCheck className="w-10 h-10 text-purple-500 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700 mb-1">{t("totalPoints")}: {score}</p>
...
export default WordProblemSprint;
