...
import { ArrowLeft, CheckCircle, Minus, Plus, Timer, XCircle } from "lucide-react";
import { useTranslation } from "../../hooks/useTranslation";
const QUESTIONS = [
  { id: 1, q: "Compute: (-3) + 7", options: ["-10", "-4", "4", "10"], correct: 2 },
  { id: 2, q: "Compute: 6 - (-2)", options: ["-8", "8", "4", "-4"], correct: 1 },
  { id: 3, q: "Compute: (-4) \xD7 (-5)", options: ["-20", "20", "9", "-9"], correct: 1 },
  { id: 4, q: "Compute: (-12) \xF7 3", options: ["-36", "-4", "4", "36"], correct: 1 },
  { id: 5, q: "Which is greater?", options: ["-2", "-5"], correct: 0 },
  { id: 6, q: "Compute: 5 + (-8)", options: ["-13", "-3", "3", "13"], correct: 1 }
];
const IntegerOps7 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("integer-ops7", scoreVal, { title: "Integer Ops7", subject: "Mathematics" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" /> {t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><div className="flex items-center justify-center mb-2"><Minus className="w-6 h-6 text-rose-600 mr-2" /><Plus className="w-6 h-6 text-emerald-600" /></div><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700 mb-1">{t("totalPoints")}: {score}</p>
