...
import { ArrowLeft, Gauge, CheckCircle, XCircle } from "lucide-react";
import { useTranslation } from "../../hooks/useTranslation";
const QUESTIONS = [
  { id: 1, q: "Probability of a sure event equals", options: ["0", "1", "1/2", "Undefined"], correct: 1 },
  { id: 2, q: "Probability of an impossible event equals", options: ["0", "1", ">1", "<0"], correct: 0 },
  { id: 3, q: "For an event E, P(E) + P(\u0305E) equals", options: ["0", "1", "2", "P(E)"], correct: 1 },
  { id: 4, q: "A fair die is rolled. P(getting an even number) =", options: ["1/6", "1/3", "1/2", "2/3"], correct: 2 },
  { id: 5, q: "A coin is tossed 100 times with 56 heads observed. Experimental P(head) \u2248", options: ["0.44", "0.50", "0.56", "0.60"], correct: 2 },
  { id: 6, q: "In a bag with 3 red and 2 blue balls, P(picking red) =", options: ["2/5", "3/5", "1/2", "3/2"], correct: 1 },
  { id: 7, q: "For a fair die, P(prime number) =", options: ["3/6", "2/6", "1/6", "5/6"], correct: 0 },
  { id: 8, q: "If P(E)=0.3, then P(\u0305E)=", options: ["0.7", "0.3", "0.5", "1.3"], correct: 0 },
  { id: 9, q: "Theoretical probability is based on", options: ["Observation only", "Equally likely outcomes", "Biased outcomes", "Past data only"], correct: 1 },
  { id: 10, q: "Experimental probability becomes closer to theoretical probability when number of trials", options: ["Decreases", "Remains same", "Increases", "Does not matter"], correct: 2 }
];
const ProbabilityBasics10 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("probability-basics10", scoreVal, { title: "Probability Basics10", subject: "Mathematics" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" />{t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><Gauge className="w-10 h-10 text-emerald-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700">{t("totalPoints")}: {score}</p>
...
export default ProbabilityBasics10;
