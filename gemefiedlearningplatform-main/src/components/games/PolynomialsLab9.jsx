...
const QUESTIONS = [
  { id: 1, q: "Identity: (a+b)^2 =", options: ["a^2 + 2ab + b^2", "a^2 - 2ab + b^2", "a^2 + b^2", "2a^2 + 2b^2"], correct: 0 },
  { id: 2, q: "Multiply: (x+2)(x+3)", options: ["x^2 + 6", "x^2 + 5x + 6", "x^2 + 5", "x^2 + 2x + 3"], correct: 1 },
  { id: 3, q: "Add: (2x^2 + 3x) + (x^2 - x + 4)", options: ["3x^2 + 2x + 4", "x^2 + 2x + 4", "3x^2 + 4x + 4", "3x^2 + 3x + 4"], correct: 2 },
  { id: 4, q: "Subtract: (5x^2 - 2x) - (3x^2 + x)", options: ["8x^2 - 3x", "2x^2 - 3x", "2x^2 - x", "8x^2 - x"], correct: 1 },
  { id: 5, q: "Factor: x^2 + 7x + 12", options: ["(x+3)(x+4)", "(x-3)(x-4)", "(x+6)(x+1)", "(x+2)(x+5)"], correct: 0 }
];
const PolynomialsLab9 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("polynomials-lab9", scoreVal, { title: "Polynomials Lab9", subject: "Mathematics" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" />{t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><Shapes className="w-10 h-10 text-rose-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700">{t("totalPoints")}: {score}</p>
...
export default PolynomialsLab9;
