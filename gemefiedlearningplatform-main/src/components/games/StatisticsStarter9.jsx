...
import { ArrowLeft, BarChart3, CheckCircle, XCircle } from "lucide-react";
import { useTranslation } from "../../hooks/useTranslation";
const QUESTIONS = [
  { id: 1, q: "Mean of 6, 8, 10, 6", options: ["6", "7", "7.5", "8"], correct: 2 },
  { id: 2, q: "Median of 2, 4, 4, 5, 9", options: ["4", "5", "2", "9"], correct: 0 },
  { id: 3, q: "For grouped data, we use", options: ["Class intervals", "Only raw list", "Pictures", "Angles"], correct: 0 },
  { id: 4, q: "Range is", options: ["Max + Min", "Max \u2212 Min", "Mean \xD7 n", "Median \u2212 Mode"], correct: 1 },
  { id: 5, q: "Bar graph best shows", options: ["Trends over time", "Proportions of a whole", "Comparisons between categories", "Angles of a polygon"], correct: 2 }
];
const StatisticsStarter9 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("statistics-starter9", scoreVal, { title: "Statistics Starter9", subject: "Science" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" />{t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><BarChart3 className="w-10 h-10 text-cyan-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700">{t("totalPoints")}: {score}</p>
...
export default StatisticsStarter9;
