...
const QUESTIONS = [
  { id: 1, q: "Solve: x^2 - 9x + 20 = 0", options: ["x=4 or 5", "x=2 or 10", "x=1 or 20", "No real roots"], correct: 0 },
  { id: 2, q: "System: x + y = 7, x - y = 1 \u21D2 x = ?", options: ["3", "4", "5", "6"], correct: 1 },
  { id: 3, q: "Factor: x^2 - 16", options: ["(x-4)(x+4)", "(x-8)(x+2)", "(x-16)(x+1)", "Prime"], correct: 0 },
  { id: 4, q: "Solve: 3x + 2y = 14, x + y = 5 \u21D2 y = ?", options: ["2", "3", "4", "5"], correct: 0 },
  { id: 5, q: "Quadratic roots are real when discriminant is", options: ["> 0", "= 0", "< 0", "\u2265 0 (real/ equal or distinct)"], correct: 3 }
];
const AlgebraMaster9 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("algebra-master9", scoreVal, { title: "Algebra Master9", subject: "Mathematics" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" />{t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><Sigma className="w-10 h-10 text-purple-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700">{t("totalPoints")}: {score}</p>
