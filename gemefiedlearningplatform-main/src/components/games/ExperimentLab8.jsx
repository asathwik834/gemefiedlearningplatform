...
import { ArrowLeft, FlaskRound, CheckCircle, XCircle } from "lucide-react";
import { useTranslation } from "../../hooks/useTranslation";
const QUESTIONS = [
  { id: 1, q: "Adding acid to water is safer than water to acid.", options: ["True", "False"], correct: 0 },
  { id: 2, q: "Indicator that turns pink in base?", options: ["Phenolphthalein", "Litmus (red)", "Turmeric", "Methyl orange"], correct: 0 },
  { id: 3, q: "Heating increases rate of reaction because", options: ["More collisions", "Less energy", "No effect", "Removes catalyst"], correct: 0 },
  { id: 4, q: "A catalyst", options: ["Is consumed", "Lowers activation energy", "Stops reaction", "Freezes solution"], correct: 1 },
  { id: 5, q: "Wear goggles and gloves in lab for", options: ["Style", "Safety", "Speed", "Colour"], correct: 1 }
];
const ExperimentLab8 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("experiment-lab8", scoreVal, { title: "Experiment Lab8", subject: "Science" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" /> {t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><FlaskRound className="w-10 h-10 text-rose-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700">{t("totalPoints")}: {score}</p>
