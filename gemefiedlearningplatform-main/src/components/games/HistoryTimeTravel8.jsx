...
import { ArrowLeft, Timer, Landmark } from "lucide-react";
import { useTranslation } from "../../hooks/useTranslation";
const QUESTIONS = [
  { id: 1, q: "Arrange earliest to latest: Maurya, Gupta, Mughal", options: ["Maurya, Gupta, Mughal", "Gupta, Maurya, Mughal", "Mughal, Gupta, Maurya", "Maurya, Mughal, Gupta"], correct: 0 },
  { id: 2, q: "Ashoka belonged to the", options: ["Maurya", "Gupta", "Mughal", "Chola"], correct: 0 },
  { id: 3, q: "Red Fort was built by", options: ["Akbar", "Shah Jahan", "Aurangzeb", "Babur"], correct: 1 },
  { id: 4, q: "Nalanda was an ancient", options: ["Temple", "University", "Fort", "Market"], correct: 1 },
  { id: 5, q: "Qutub Minar is in", options: ["Agra", "Delhi", "Jaipur", "Mumbai"], correct: 1 }
];
const HistoryTimeTravel8 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("history-time-travel8", scoreVal, { title: "History Time Travel8", subject: "Science" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" /> {t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><Landmark className="w-10 h-10 text-indigo-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700">{t("totalPoints")}: {score}</p>
  ...
  return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><div className="flex items-center justify-between mb-6"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800"><ArrowLeft className="w-5 h-5 mr-2" /> {t("backToGames")}</button><div className="hidden sm:block text-sm text-gray-700">{t("player") || "Player"}: {currentUser}</div></div><div className="bg-white rounded-xl shadow-sm p-8"><h3 className="text-xl font-semibold text-gray-900 mb-4 flex items-center"><Landmark className="w-5 h-5 mr-2" /> {q.q}</h3><div className="grid grid-cols-1 gap-3">{q.options.map((opt, i) => {
    ...
    return <button key={i} className={cls} onClick={() => handleSelect(i)} disabled={show}><div className="flex items-center justify-between"><span>{opt}</span><Timer className="w-4 h-4 text-gray-400" /></div></button>;
  })}</div></div></div></div>;
};
export default HistoryTimeTravel8;
