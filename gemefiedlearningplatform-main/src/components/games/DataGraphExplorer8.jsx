...
const QUESTIONS = [
  { id: 1, q: "A bar chart shows values 5, 8, 6, 12. Which is highest?", options: ["5", "6", "8", "12"], correct: 3 },
  { id: 2, q: "Line graph rises steadily. What does it suggest?", options: ["Decrease", "No change", "Increase", "Random"], correct: 2 },
  { id: 3, q: "Pie chart: 25% red, 50% blue, 25% green. Which is most?", options: ["Red", "Blue", "Green", "All equal"], correct: 1 },
  { id: 4, q: "If a data set is 2, 4, 4, 6, 8, the mode is", options: ["2", "4", "6", "8"], correct: 1 },
  { id: 5, q: "For 10 students: avg marks 60. Total marks is", options: ["6", "60", "600", "610"], correct: 2 }
];
const DataGraphExplorer8 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("data-graph-explorer8", scoreVal, { title: "Data Graph Explorer8", subject: "Science" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" /> {t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><BarChart3 className="w-10 h-10 text-indigo-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700">{t("totalPoints")}: {score}</p>
  ...
  return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><div className="flex items-center justify-between mb-6"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800"><ArrowLeft className="w-5 h-5 mr-2" /> {t("backToGames")}</button><div className="hidden sm:block text-sm text-gray-700">{t("player") || "Player"}: {currentUser}</div></div><div className="bg-white rounded-xl shadow-sm p-8"><h3 className="text-xl font-semibold text-gray-900 mb-4 flex items-center"><LineChart className="w-5 h-5 mr-2" /> {q.q}</h3><div className="grid grid-cols-1 gap-3">{q.options.map((opt, i) => {
    ...
    return <button key={i} className={cls} onClick={() => handleSelect(i)} disabled={show}><div className="flex items-center justify-between"><span>{opt}</span><PieChart className="w-4 h-4 text-gray-400" /></div></button>;
  })}</div></div></div></div>;
};
export default DataGraphExplorer8;
