...
const QUESTIONS = [
  { id: 1, q: "Ohm's law states V =", options: ["IR", "I/R", "R/I", "I^2 R"], correct: 0 },
  { id: 2, q: "For resistors in series, equivalent resistance is", options: ["R1 + R2 + ...", "1/(1/R1 + 1/R2 + ...)", "R1R2", "max(R1,R2)"], correct: 0 },
  { id: 3, q: "For resistors in parallel, equivalent resistance is", options: ["R1 + R2", "R1R2/(R1+R2)", "R1 - R2", "R1R2R3"], correct: 1 },
  { id: 4, q: "Power consumed P equals", options: ["VI", "I^2 R", "V^2/R", "All of these (context dependent)"], correct: 3 },
  { id: 5, q: "A 2 \u03A9 and 3 \u03A9 resistor in series have Req =", options: ["1.2 \u03A9", "5 \u03A9", "0.83 \u03A9", "2.5 \u03A9"], correct: 1 },
  { id: 6, q: "A 2 \u03A9 and 3 \u03A9 resistor in parallel have Req \u2248", options: ["5 \u03A9", "2.5 \u03A9", "1.2 \u03A9", "0.4 \u03A9"], correct: 2 },
  { id: 7, q: "If V = 12 V and R = 4 \u03A9, current I =", options: ["2 A", "3 A", "4 A", "8 A"], correct: 1 },
  { id: 8, q: "Fuse protects circuits by", options: ["Increasing current", "Breaking the circuit when current is high", "Storing charge", "Lowering voltage"], correct: 1 },
  { id: 9, q: "In household wiring, devices are connected mostly in", options: ["Series", "Parallel", "Series-Parallel", "None"], correct: 1 },
  { id: 10, q: "If I = 2 A through 5 \u03A9, power P =", options: ["5 W", "10 W", "20 W", "40 W"], correct: 2 }
];
const ElectricityCircuits10 = ({ onBack, currentUser }) => {
  ...
  const handleSubmitScore = async () => {
    try {
      ...
      await rewards.recordHighScore("electricity-circuits10", scoreVal, { title: "Electricity Circuits10", subject: "Science" });
      setSubmitted(true);
    } catch (err) {
  ...
  if (done) return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4"><div className="max-w-2xl mx-auto"><button onClick={onBack} className="flex items-center text-blue-600 hover:text-blue-800 mb-6"><ArrowLeft className="w-5 h-5 mr-2" />{t("backToGames")}</button><div className="bg-white rounded-xl shadow p-8 text-center"><Zap className="w-10 h-10 text-emerald-600 mx-auto mb-2" /><h2 className="text-2xl font-bold mb-2">{t("gameComplete")}</h2><p className="text-gray-700">{t("totalPoints")}: {score}</p>
...
export default ElectricityCircuits10;
